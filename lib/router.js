...
Router.route('/tarot', function(){
   this.render('tarot')
});

Router.route('/radio', function(){
  Router.go('/radio/about');
});

Router.route('/radio/:nav', function () {
  if (isMobile()) {
    this.layout('radioMobile');
    this.render(this.params.nav, {to: 'mobileRadioBody'});
    }else{
    this.layout('radioBrowser');
    this.render(this.params.nav, {to: 'mobileRadioBody'});
